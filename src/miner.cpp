...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DLRMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("DLRMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("DLRMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("delor-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("DLRMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running DLRMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadDLRMiner() exception");
    } catch (...) {
        LogPrintf("ThreadDLRMiner() exception");
    }

    LogPrintf("ThreadDLRMiner exiting\n");
}
